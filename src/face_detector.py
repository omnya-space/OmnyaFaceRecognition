...
class FaceDetector(object):
    ...
    def detect_face(self, img, det_type=0):
        ...
        # print ("original boxes", total_boxes)
        # print ("original points", points[0].reshape((2, 5)).T)
        return total_boxes, points
    ...
    def get_points_and_bbox(self, face_img):

        ret = self.detect_face(face_img)
        if ret is None:
            return None
        ...
        for i in range(bbox.shape[0]):
            ...
            confidence = bbox[i][4]
            det = bbox[i][0:4]
            nimg = face_preprocess.preprocess(face_img, det, landmarks, image_size='112,112')
            nimg = cv2.cvtColor(nimg, cv2.COLOR_BGR2RGB)

            fm = cv2.Laplacian(nimg, cv2.CV_64F).var()
            bnw = cv2.cvtColor(nimg, cv2.COLOR_RGB2GRAY)


            alignedImg = np.transpose(nimg, (2, 0, 1))
            ...
            aligned.append([alignedImg, rect])
